...
class Viewer3D {
        ...
        this.ifcapi.StreamAllMeshes(modelID, (mesh) => {
            ...
            for (let gi = 0; gi < mesh.geometries.size(); gi++) {
                ...
                for (let j = 0; j < verts.length; j += 6) {
                    ...
                    // All-bars bbox — used for overall width display on the website
                    if (bar) {
    ...
    _buildDimensions() {
        ...
        const meshWidth  = Math.round(Math.min(spanX, spanZ)); // mesh bars outer-to-outer (for Excel/EDB)
        const meshLength = Math.round(Math.max(spanX, spanZ));

        // Overall dims from all bars (for website display)
